...
typedef struct {
  ...
  // Cached value to skip redundant memcpy calls
  // Max size: coord (3 doubles = 24 bytes) or vec4 (4 floats = 16 bytes)
  char cached[24];
  bool cached_valid;
#endif
} MyUniform;
...
#ifdef __HIP_PLATFORM_AMD__
  hipDeviceptr_t cached_ssbo;  // cached _data_ptr value
#endif
};
...
#ifdef __HIP_PLATFORM_AMD__
  // AMD ROCm - use hipMemset for floats (4 bytes at a time)
                      ...
                      unsigned ng[2], unsigned nwg[2])
{
...
#ifdef __HIP_PLATFORM_AMD__
  /**
  Get the _data_ptr device global (AMD uses __device__, not __constant__). */
  ...
  // Note: csOrigin is declared without __constant__, so hipModuleGetGlobal works on AMD too
...
void post_setup_shader (Shader * shader, External * externals)
{
  ...
  /**
  Set SSBO pointer (_data_ptr on AMD, _data on NVIDIA). */
#ifdef __HIP_PLATFORM_AMD__
  // Only copy if ssbo changed
  if (shader->cached_ssbo != ssbo) {
    HIP_CHECK (hipMemcpyAsync ((void *)shader->_data, &ssbo, sizeof (ssbo), hipMemcpyHostToDevice, stream));
    shader->cached_ssbo = ssbo;
  }
#else
  ...
  for (const MyUniform * g = shader->uniforms; g && g->type; g++) {
    ...
    case sym_INT: case sym_FLOAT: case sym_VEC4: case sym_BOOL:
      ...
      if (g->cached_valid && memcmp (g->cached, pointer, g->size) == 0)
        break;
      memcpy (g->cached, pointer, g->size);
      g->cached_valid = true;
      HIP_CHECK (hipMemcpyAsync ((void *)g->location, pointer, g->size, hipMemcpyHostToDevice, stream));
#else
	...
	p[i] = data[i];
#ifdef __HIP_PLATFORM_AMD__
      if (g->cached_valid && memcmp (g->cached, p, g->size) == 0)
        break;
      memcpy (g->cached, p, g->size);
      g->cached_valid = true;
      HIP_CHECK (hipMemcpyAsync ((void *)g->location, p, g->size, hipMemcpyHostToDevice, stream));
#else
...
int run_shader (const Shader * shader, const RegionParameters * region)
{
...
#ifdef __HIP_PLATFORM_AMD__
    // AMD: csOrigin not supported (device global not accessible)
    // For foreach_point(), we need to pass origin as kernel arguments
    // For now, this code path is not fully supported on AMD
    void * args[] = { &ssbo };
  ...
  This is a region */
    ...
    assert (!GPUContext.fragment_shader);
#ifdef __HIP_PLATFORM_AMD__
    // AMD: pass ssbo as kernel argument
    void * args[] = { &ssbo };
    HIP_CHECK (hipModuleLaunchKernel (shader->kernel,
...
#else
      HIP_CHECK (hipMemFree ((void *) d_output_a));
      HIP_CHECK (hipMemFree ((void *) d_output_b));
#endif
    }
#ifdef __HIP_PLATFORM_AMD__
...
#else
    HIP_CHECK (hipMemAlloc ((void **) &d_output_a, max_blocks*sizeof(float)));
    HIP_CHECK (hipMemAlloc ((void **) &d_output_b, max_blocks*sizeof(float)));
#endif
    Np = N;
  }
