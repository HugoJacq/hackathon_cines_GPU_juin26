...
#if _CUDA
#  if _HIP_AMD
// HIP on AMD ROCm - use pure aggregate structs (no constructors)
// With aggregate structs, __constant__ storage works because {a,b} is static init, not dynamic
// __constant__ memory is visible to hipModuleGetGlobal
#    define DEFINITIONS              \
...
const char glsl_preproc[] =
  ...
  DEFINITIONS
#if _CUDA
  // Both HIP/AMD and CUDA/NVIDIA: use brace initialization with aggregate structs
  ...
  // Pure aggregate struct for all platforms (CUDA, HIP, OpenGL) - brace initialization works
...
#if _CUDA
#  if _HIP_AMD
  // HIP/AMD: use __device__ for variables set via hipModuleGetGlobal + hipMemcpy
  "__device__ ivec2 csOrigin = {0,0};\n"
  "__device__ vec2 vsOrigin = {0.f,0.f};\n"
  "__device__ vec2 vsScale = {1.f,1.f};\n"
#  else
  // CUDA/NVIDIA: use __constant__ (works with cuModuleGetGlobal)
  "__constant__ ivec2 csOrigin = {0,0};\n"
  "__constant__ vec2 vsOrigin = {0.f,0.f};\n"
  "__constant__ vec2 vsScale = {1.f,1.f};\n"
#  endif
#else
  // OpenGL GLSL
...
#if _CUDA
#  if _HIP_AMD
                    // HIP/AMD: _data_ptr is a __device__ global set via hipMemcpy before kernel launch
...
#  endif
...
#if _HIP_AMD
    fs = str_append (fs,
		     "#define _data_val(field,index) _data_ptr[(field)*field_size() + (index)]\n");
#else
    fs = str_append (fs, ";\n"
  ...
  for (External * g = externals; g; g = g->next) {
	...
	fs = str_append (fs,
                         // Use brace initialization for all platforms
...
#if _HIP_AMD
	// HIP/AMD: declare as __device__ variable (set via hipMemcpyAsync)
	fs = str_append (fs, "__device__ ", type, " ", EXTERNAL_NAME (g));
	for (int * d = g->data; d && *d > 0; d++) {
	  char s[20]; snprintf (s, 19, "%d", *d);
	  fs = str_append (fs, "[", s, "]");
	}
...
static char * shader_append_func (char * s, const ForeachData * loop)
{
  ...
  sprintf (func, "%s_%d", loop->func, loop->line);
#  if _HIP_AMD
  // HIP/AMD: _data_ptr is set via hipMemcpy before kernel launch
  // We still accept it as a parameter for compatibility but don't use it
  return str_append (s,
                     "extern \"C\" __global__\n"
...
{  
  ...
  if (!GPUContext.fragment_shader) {
    ...
    shader = str_append (shader,
#if _CUDA
    // CUDA and HIP: use brace initialization with aggregate structs
