...
## Performance Optimization: Uniform Caching

### Issue 10: High `hipMemcpyAsync` Overhead

**Symptom:** `setup_shader` consuming 64% of runtime due to ~2.5M `hipMemcpyAsync` calls (6+ uniforms × 400k kernel launches).

**Cause:** Every kernel launch copies all uniform values to device, even when values haven't changed.

**Resolution:** Added per-uniform caching to skip redundant copies:
...
typedef struct {
  ...
  char cached[24];  // Max: coord = 3 doubles = 24 bytes
  bool cached_valid;
#endif
} MyUniform;
```
...
Also cached the `_data_ptr` (SSBO pointer) which rarely changes:
```c
#ifdef __HIP_PLATFORM_AMD__
...
This optimization significantly reduces the number of `hipMemcpyAsync` calls since most uniforms (L0, X0, Y0, etc.) remain constant throughout the simulation, while only `dt` changes occasionally.
